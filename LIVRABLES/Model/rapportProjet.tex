...
\begin{document}
	...
	\item \textbf{Faasd :} Sur le Raspberry Pi, nous avons utilisé \texttt{faasd}, une version légère et simplifiée d’OpenFaaS. Faasd ne nécessite pas de cluster Kubernetes et fonctionne sur un seul hôte, ce qui le rend parfaitement adapté aux environnements contraints comme le Raspberry Pi. Il permet d’exécuter des fonctions serverless localement avec un minimum de ressources, tout en conservant les avantages du modèle FaaS (isolation des fonctions, scalabilité, déploiement simple via Docker).
	...
	\subsection{Structure}
		\begin{figure}[ht]
			\centering
			\includegraphics[scale=0.25]{architecture3.jpg}
	...
	\begin{enumerate}
		...
		Cette fonction gère l'envoi des données collectées depuis le Raspberry Pi vers le serveur distant. Le fichier JSON contenant les informations de détection, ainsi que le dossier des images, sont envoyés via le protocole NATS. L’envoi est déclenché soit à intervalles réguliers (toutes les 24 heures), soit dès qu’un seuil de 10 nouvelles images est atteint.\\
		...
		\begin{figure}[ht]
			...
			\caption{Pré-traitement vs traitement final}
			\label{fig3}
		\end{figure}		
	...
	Globalement, notre système a permis de détecter et classifier plusieurs espèces animales avec une précision satisfaisante, tout en assurant une gestion optimisée des ressources embarquées et une bonne capacité d’extension côté serveur. Les tests réalisés ont confirmé la viabilité de notre approche, tant sur le plan technique que fonctionnel.
	...
	Nous avons également rencontré des difficultés lors de l'entraînement de notre propre modèle YOLO, dans le but de l’améliorer en le fusionnant avec le modèle préexistant. Malheureusement, cette opération s’est révélée impossible : YOLO ne permet pas de fusionner directement un modèle personnalisé avec un modèle de base. Nous avons donc dû entraîner un modèle complet, en collectant et en annotant des images pour plusieurs espèces animales. Ce processus, particulièrement chronophage, a nécessité des efforts importants pour constituer un jeu de données représentatif et atteindre un niveau de précision acceptable.\\
	...
	\section{Conclusion}
	Ce projet nous a permis de concevoir, développer et tester une solution embarquée de détection d’animaux intégrant des technologies modernes et interopérables, telles que YOLO, OpenFaaS/faasd, NATS et SQLite. L’objectif initial – surveiller une zone naturelle de manière automatisée, tout en limitant les ressources utilisées – a été atteint. Par manque de temps et dû a des contraintes d'emploi du temps, nous n'avons malheureusement pas pu intégrer notre solution au projet Mycelium.
	...
	\begin{itemize}
		\item L'amélioration de la caméra pour avoir des images de meilleures qualités ainsi que l'utilisation d'une caméra infrarouge pour pouvoir faire des détections nocturnes.
